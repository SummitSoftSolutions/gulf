...
class AccountGroupMaster extends StatefulWidget {
  const AccountGroupMaster({super.key});

  @override
  State<AccountGroupMaster> createState() => _AccountGroupMasterState();
}

class _AccountGroupMasterState extends State<AccountGroupMaster> {
  var apiUrl;
  ...
  TextEditingController codeCntrg = TextEditingController();
  TextEditingController nameg = TextEditingController();
  bool showUpdateButton = false;
  String? selectedaccountval1;
  var account_val;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: Colors.white,
        appBar: AppBar(backgroundColor:HexColor('#3465D9'),title: const Text('Account Group Master',style: TextStyle(color: Colors.white),),automaticallyImplyLeading: true,leading: IconButton(
                        ...
                        Padding(
                          ...
                          child: Container(
                            ...
                            child: DropdownSearch<String>(
                                selectedItem: selectedaccountval1,
                        ...
                        Row(
                          children: [
                            GestureDetector(
                              onTap: () {
                                setState(() {
                                  isSelected = !isSelected; // Toggle the selection state for the first toggle button
                                });
                              },
                              child: Row(
                                children: [
                                  Container(
                                    ...
                                    decoration: BoxDecoration(
                                      ...
                                      color: isSelected ? Colors.blue : Colors.transparent, // Set color based on selection state
                                      border: Border.all(color:HexColor('#3465D9'),),
                                    ),
                                    child: isSelected
                                        ? Icon(Icons.check, size: 16, color: HexColor('#3465D9'),) // Show check icon when selected
                                        : null,
                                  ),
  ...
  Future<void> EditUserData() async {
    ...
    var response = await http.post(
      Uri.parse('$apiUrl/AccountGroupMaster_edit/'),
      body: {
        'name': nameg.text,
        'accid': accidval,
        'parentid':parentidval,
        'selectedaccountval1':selectedUserID1,
      },
    );
  ...
  Future fetchaccount() async {
    ...
    if (response.statusCode == 200) {
      setState(() {
        account_val = jsonDecode(response.body);
      });
  ...
  Future insertgroupaccount() async {
    await getApiurl();
    var response = await http.post(
        (Uri.parse(
            '$apiUrl/insert_accountgroupmaster/')),
        body: {
          'codeCntr':codeCntrg.text,
          'nameg':nameg.text,
          'selectedaccountval1':selectedUserID1,
          'selectedValueg':isSelected.toString(),

        }
    );
    var data;
    print("insertgroupaccount");
    print(response.body);
    if (response.statusCode == 200) {
      setState(() {
        data = response.body.trim(); // No need for jsonDecode
  ...
  Future showEditZoneForm(Map<String, dynamic> zoneData) async {
    ...
    print("clikcked fun");
    nameg.text = zoneData['name'];
    selectedaccountval1 = zoneData['parentid__name'].toString();
      ...
      if (decodedResponse is List<dynamic> && decodedResponse.isNotEmpty) {
        ...
        if (subreg1 != null) {
          ...
          if(subreg1 as int<100)
          {
            ...
            setState(() {
              codeCntrg.text = incrementedValue.toString(); // Set the incremented value to regval.text
            });
  ...
  Future<List<Map<String, dynamic>>> fetchallArea() async {
    ...
    await getApiurl(); // Assuming this function sets the API URL
    var response = await http.post(Uri.parse('$apiUrl/viewAllAccountgroupmaster/'));
