...
class NotRenew extends StatefulWidget {
  final String? id;

  const NotRenew({Key? key, this.id}) : super(key: key);
  @override
  State<NotRenew> createState() => _NotRenewState();
}

class _NotRenewState extends State<NotRenew> {
  bool isRenewed = false; // State variable to track if renewed
    ...
    return  WillPopScope(
      onWillPop: () async {
        Navigator.pushAndRemoveUntil(
          context,
          MaterialPageRoute(builder: (context) => NotificationAgent(firstname: firstname,id: id, isAdmin:false)),
        ...
        appBar: AppBar(backgroundColor:HexColor('#3465D9'),title: const Padding(
          ...
          onPressed: () {
            Navigator.pushAndRemoveUntil(
              context,
              MaterialPageRoute(
                builder: (context) => NotificationAgent(
                  firstname: firstname,
                  id: id,
                  isAdmin: isAdmin,
                ),
                          ...
                          child: Row(
                            children: [
                              Padding(
                                ...
                                child: Text('Balance: $balance',style: const TextStyle(fontSize: 15,fontWeight: FontWeight.bold),),
                              ),
                            ],
                          ),
                        ),
                                  ...
                                  child: Container(
                                    ...
                                    child: DropdownButton<String>(
                                      ...
                                      onChanged: (String? newValue) {// Retrieve the selected value here
                                        setState(() {
                                          if (newValue == null) {
                                            selectedValue = ''; // Set selectedValue to empty if newValue is null
                                            isShow = false;
                                          }
                                            ...
                                            child:  DropdownSearch<String>(
                                              ...
                                              onChanged: (value) {
                                                setState(() {
                                                  ...
                                                  {
                                                    checkVisible=true;// Assign only if the value is a String
                                                  }
                                                  else
                                                  {
                                                    checkVisible=false;
  ...
  //   }
  // }
  Future<void> selectsubcriber() async {
    print("selectsubcriber.............................");

    await getApiurl(); // Assuming this function sets the API URL
    var response = await http.post(Uri.parse('$apiUrl/SelectSubNot/'), body: {
      'id': widget.id
    });
    ...
    if (response.statusCode == 200) {
      ...
      users = (user_val as List).map<UserListData>((model) {
        return UserListData.fromJson(model);
      }).toList();

      print("users................$users");

      // Set the initial value for selecteduser if users is not empty
      if (users.isNotEmpty) {
        selecteduser = users[0].name; // Initialize with the first user name or any specific logic
        subscriberid = users[0].id.toString(); // Set the initial ID
        selectedUserID = users[0].accountid.toString(); // Set the initial account ID
        selectedUserID1 = users[0].regid; // Set the initial reg ID
      }
    } else {
      // Handle API error
  ...
  Future<void> renewal_insert() async {
    ...
    print("finalCoordinatorId...................$finalCoordinatorId");
    var response = await http.post(
